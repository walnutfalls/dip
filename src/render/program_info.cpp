...
render::program_info::program_info(const gl::GLuint programId)
{
	_id = programId;
...
gl::GLuint render::program_info::getAttribLocation(const std::string& name) const
{
	return _attribLocations.at(name);
}

gl::GLuint render::program_info::getUniformLocation(const std::string& name) const
{
	if (_uniformLocations.find(name) == _uniformLocations.end()) return -1;
	return _uniformLocations.at(name);
}

gl::GLuint render::program_info::getUniformBlockLocation(const std::string& name) const
{
	if (_uniformLocations.find(name) == _uniformLocations.end()) return -1;
